...
function(dr) { return "/*##1##*/" +  cat([makeStatement(dr), " const ", makeId(dr), ";"]); },

function(dr) { return "/*##2##*/" +  cat([makeStatement(dr), makeStatement(dr)]); },
function(dr) { return "/*##3##*/" +  cat([makeStatement(dr-1), "\n", makeStatement(dr-1), "\n"]); },
...
function(dr) { return "/*##4##*/" +  cat([stripSemicolon(makeStatement(dr)), "\n", makeStatement(dr)]); },
function(dr) { return "/*##5##*/" +  cat([stripSemicolon(makeStatement(dr)), "\n"                   ]); },
function(dr) { return "/*##6##*/" +  stripSemicolon(makeStatement(dr)); }, // usually invalid, but can be ok e.g. at the end of a block with curly braces

// Blocks and loops
function(dr) { return "/*##7##*/" +  cat(["{", makeStatement(dr), " }"]); },
function(dr) { return "/*##8##*/" +  cat(["{", makeStatement(dr-1), makeStatement(dr-1), " }"]); },

function(dr) { return "/*##9##*/" +  cat([maybeLabel(), "with", "(", makeExpr(dr), ")", makeStatementOrBlock(dr)]); },
function(dr) { return "/*##10##*/" +  cat([maybeLabel(), "with", "(", "{", makeId(dr), ": ", makeExpr(dr), "}", ")", makeStatementOrBlock(dr)]); },
...
function(dr) { return "/*##11##*/" +  "/*infloop*/" + cat([maybeLabel(), "for", "(", makeExpr(dr), "; ", makeExpr(dr), "; ", makeExpr(dr), ") ", makeStatementOrBlock(dr)]); },
function(dr) { return "/*##12##*/" +  "/*infloop*/" + cat([maybeLabel(), "for", "(", rndElt(varBinder), makeId(dr),                                       "; ", makeExpr(dr), "; ", makeExpr(dr), ") ", makeStatementOrBlock(dr)]); },
function(dr) { return "/*##13##*/" +  "/*infloop*/" + cat([maybeLabel(), "for", "(", rndElt(varBinder), makeDestructuringLValue(dr), " = ", makeExpr(dr), "; ", makeExpr(dr), "; ", makeExpr(dr), ") ", makeStatementOrBlock(dr)]); },

// "for..in" loops

// -- for (key in obj)
function(dr) { return "/*##14##*/" +  "/*for..in*/" + cat([maybeLabel(), "for", "(", rndElt(varBinder), makeForInLHS(dr), " in ", makeExpr(dr-2), ") ", makeStatementOrBlock(dr)]); },
// -- for (key in generator())
function(dr) { return "/*##15##*/" +  "/*for..in*/" + cat([maybeLabel(), "for", "(", rndElt(varBinder), makeForInLHS(dr), " in ", "(", "(", makeFunction(dr), ")", "(", makeExpr(dr), ")", ")", ")", makeStatementOrBlock(dr)]); },
// -- for each (value in obj)
function(dr) { return "/*##16##*/" +  "/*for..in*/" + "/* nogeckoex bug 349964 */" + cat([maybeLabel(), " for ", " each", "(", rndElt(varBinder), makeLValue(dr), " in ", makeExpr(dr-2), ") ", makeStatementOrBlock(dr)]); },
...
function(dr) { return "/*##17##*/" +  "/*for..in*/" + cat([maybeLabel(), "for", "(", rndElt(varBinder), makeId(dr), " = ", makeExpr(dr), " in ", makeExpr(dr-2), ") ", makeStatementOrBlock(dr)]); },
function(dr) { return "/*##18##*/" +  "/*for..in*/" + cat([maybeLabel(), "for", "(", rndElt(varBinder), "[", makeId(dr), ", ", makeId(dr), "]", " = ", makeExpr(dr), " in ", makeExpr(dr-2), ") ", makeStatementOrBlock(dr)]); },

function(dr) { return "/*##19##*/" +  cat([maybeLabel(), "while((", makeExpr(dr), ") && 0)" /*don't split this, it's needed to avoid marking as infloop*/, makeStatementOrBlock(dr)]); },
function(dr) { return "/*##20##*/" +  "/*infloop*/" + cat([maybeLabel(), "while", "(", makeExpr(dr), ")", makeStatementOrBlock(dr)]); },
function(dr) { return "/*##21##*/" +  cat([maybeLabel(), "do ", makeStatementOrBlock(dr), " while((", makeExpr(dr), ") && 0)" /*don't split this, it's needed to avoid marking as infloop*/, ";"]); },
function(dr) { return "/*##22##*/" +  "/*infloop*/" + cat([maybeLabel(), "do ", makeStatementOrBlock(dr), " while", "(", makeExpr(dr), ");"]); },

// Switch statement
function(dr) { return "/*##23##*/" +  cat([maybeLabel(), "switch", "(", makeExpr(dr), ")", " { ", makeSwitchBody(dr), " }"]); },

// Let blocks, with and without multiple bindings, with and without initial values
function(dr) { return "/*##24##*/" +  cat(["let ", "(", makeLetHead(dr), ")", " { ", makeStatement(dr), " }"]); },

// Conditionals, perhaps with 'else if' / 'else'
function(dr) { return "/*##25##*/" +  cat([maybeLabel(), "if(", makeExpr(dr), ") ", makeStatementOrBlock(dr)]); },
function(dr) { return "/*##26##*/" +  cat([maybeLabel(), "if(", makeExpr(dr), ") ", makeStatementOrBlock(dr-1), " else ", makeStatementOrBlock(dr-1)]); },
function(dr) { return "/*##27##*/" +  cat([maybeLabel(), "if(", makeExpr(dr), ") ", makeStatementOrBlock(dr-1), " else ", " if ", "(", makeExpr(dr), ") ", makeStatementOrBlock(dr-1)]); },
function(dr) { return "/*##28##*/" +  cat([maybeLabel(), "if(", makeExpr(dr), ") ", makeStatementOrBlock(dr-1), " else ", " if ", "(", makeExpr(dr), ") ", makeStatementOrBlock(dr-1), " else ", makeStatementOrBlock(dr-1)]); },
...
function(dr) { return "/*##29##*/" +  cat([maybeLabel(), "if(", makeExpr(dr), ") ", "{", " if ", "(", makeExpr(dr), ") ", makeStatementOrBlock(dr-1), " else ", makeStatementOrBlock(dr-1), "}"]); },
function(dr) { return "/*##30##*/" +  cat([maybeLabel(), "if(", makeExpr(dr), ") ", "{", " if ", "(", makeExpr(dr), ") ", makeStatementOrBlock(dr-1), "}", " else ", makeStatementOrBlock(dr-1)]); },

// Exception-related statements :)
function(dr) { return "/*##31##*/" +  makeExceptionyStatement(dr-1); makeExceptionyStatement(dr-1); },
function(dr) { return "/*##31##*/" +  makeExceptionyStatement(dr-1); makeExceptionyStatement(dr-1); },
function(dr) { return "/*##32##*/" +  makeExceptionyStatement(dr); },
function(dr) { return "/*##32##*/" +  makeExceptionyStatement(dr); },
function(dr) { return "/*##32##*/" +  makeExceptionyStatement(dr); },
function(dr) { return "/*##32##*/" +  makeExceptionyStatement(dr); },
function(dr) { return "/*##32##*/" +  makeExceptionyStatement(dr); },
...
function(dr) { return "/*##33##*/" +  cat(["L", ": ", makeStatementOrBlock(dr)]); },
...
[
// Tiny
function(dr) { return "/*##34##*/" +  cat([";"]); }, // e.g. empty "if" block
function(dr) { return "/*##35##*/" +  cat(["{", "}"]); ; }, // e.g. empty "if" block
function(dr) { return "/*##36##*/" +  cat([""]); },

// Force garbage collection
function(dr) { return "/*##37##*/" +  "gc()"; },

// Throw stuff.
function(dr) { return "/*##38##*/" +  cat(["throw ", makeExpr(dr), ";"]); },

// Break/continue [to label].
function(dr) { return "/*##39##*/" +  cat([rndElt(["continue", "break"]), " ", rndElt(["L", "M", "", ""]), ";"]); },

// Import and export.  (I have not idea what these actually do.)
function(dr) { return "/*##40##*/" +  cat(["export ", makeId(dr), ";"]); },
function(dr) { return "/*##41##*/" +  cat(["export ", "*", ";"]); },
...
function(dr) { return "/*##42##*/" +  makeFunction(dr); },

// Return, yield
function(dr) { return "/*##43##*/" +  cat(["return ", makeExpr(dr), ";"]); },
function(dr) { return "/*##44##*/" +  "return;"; }, // return without a value is allowed in generators; return with a value is not.
function(dr) { return "/*##45##*/" +  cat(["yield ", makeExpr(dr), ";"]); }, // note: yield can also be a left-unary operator, or something like that
function(dr) { return "/*##46##*/" +  "yield;"; },

// Expression statements
function(dr) { return "/*##47##*/" +  cat([makeExpr(dr), ";"]); },
function(dr) { return "/*##48##*/" +  cat(["(", makeExpr(dr), ")", ";"]); },

// Various kinds of variable declarations, with and without initial values (assignment).
function(dr) { return "/*##49##*/" +  cat([rndElt(varBinder), makeLetHead(dr), ";"]); }, // e.g. "const [a,b] = [3,4];"
function(dr) { return "/*##49##*/" +  cat([rndElt(varBinder), makeLetHead(dr), ";"]); }, // e.g. "const [a,b] = [3,4];"
function(dr) { return "/*##49##*/" +  cat([rndElt(varBinder), makeLetHead(dr), ";"]); }, // e.g. "const [a,b] = [3,4];"
];
...
function(dr) { return "/*##50##*/" +  makeStatement(dr); },
function(dr) { return "/*##50##*/" +  makeStatement(dr); },
function(dr) { return "/*##51##*/" +  cat(["{", makeStatement(dr), " }"]); },
function(dr) { return "/*##52##*/" +  cat(["{", makeStatement(dr-1), makeStatement(dr-1), " }"]); },
]
...
function(dr) { return "/*##53##*/" +  makeTryBlock(dr); },

function(dr) { return "/*##54##*/" +  makeStatement(dr); },
function(dr) { return "/*##55##*/" +  makeLittleStatement(dr); },

function(dr) { return "/*##56##*/" +  "return;" }, // return without a value can be mixed with yield
function(dr) { return "/*##57##*/" +  cat(["return ", makeExpr(dr), ";"]); },
function(dr) { return "/*##58##*/" +  cat(["yield ", makeExpr(dr), ";"]); },
function(dr) { return "/*##59##*/" +  cat(["throw ", makeId(dr), ";"]); },
function(dr) { return "/*##60##*/" +  "throw StopIteration;"; },
function(dr) { return "/*##61##*/" +  "this.zzz.zzz;"; }, // throws; also tests js_DecompileValueGenerator in various locations
function(dr) { return "/*##62##*/" +  cat([makeId(dr), " = ", makeId(dr), ";"]); },
function(dr) { return "/*##63##*/" +  cat([makeLValue(dr), " = ", makeId(dr), ";"]); },
...
function(dr) { return "/*##64##*/" +  "for(let y in []);"; },
function(dr) { return "/*##65##*/" +  "for(let y in [5,6,7,8]) " + makeExceptionyStatement(dr); },

// Brendan says these are scary places to throw: with, let block, lambda called immediately in let expr.
// And I think he was right.
function(dr) { return "/*##66##*/" +  "with({}) "   + makeExceptionyStatement(dr);         },
function(dr) { return "/*##67##*/" +  "with({}) { " + makeExceptionyStatement(dr) + " } "; },
function(dr) { return "/*##68##*/" +  "let(" + makeLetHead(dr) + ") { " + makeExceptionyStatement(dr); + "}"},
function(dr) { return "/*##69##*/" +  "let(" + makeLetHead(dr) + ") ((function(){" + makeExceptionyStatement(dr) + "})());" },
...
function(dr) { return "/*##70##*/" +  cat([rndElt(leftUnaryOps), makeExpr(dr)]); },
function(dr) { return "/*##70##*/" +  cat([rndElt(leftUnaryOps), makeExpr(dr)]); },
function(dr) { return "/*##70##*/" +  cat([rndElt(leftUnaryOps), makeExpr(dr)]); },

// Right-unary operators
function(dr) { return "/*##71##*/" +  cat([makeExpr(dr), rndElt(rightUnaryOps)]); },
function(dr) { return "/*##71##*/" +  cat([makeExpr(dr), rndElt(rightUnaryOps)]); },
function(dr) { return "/*##71##*/" +  cat([makeExpr(dr), rndElt(rightUnaryOps)]); },

// Special properties: we love to set them!
function(dr) { return "/*##72##*/" +  cat([makeExpr(dr), ".", rndElt(specialProperties)]); },
function(dr) { return "/*##73##*/" +  cat([makeExpr(dr), ".", rndElt(specialProperties), " = ", makeExpr(dr)]); },
function(dr) { return "/*##74##*/" +  cat([makeId(dr),   ".", rndElt(specialProperties), " = ", makeExpr(dr)]); },

// Binary operators
function(dr) { return "/*##75##*/" +  cat([makeExpr(dr), rndElt(binaryOps), makeExpr(dr)]); },
function(dr) { return "/*##75##*/" +  cat([makeExpr(dr), rndElt(binaryOps), makeExpr(dr)]); },
function(dr) { return "/*##75##*/" +  cat([makeExpr(dr), rndElt(binaryOps), makeExpr(dr)]); },
function(dr) { return "/*##75##*/" +  cat([makeExpr(dr), rndElt(binaryOps), makeExpr(dr)]); },
function(dr) { return "/*##75##*/" +  cat([makeExpr(dr), rndElt(binaryOps), makeExpr(dr)]); },
...
// Ternary operator
function(dr) { return "/*##76##*/" +  cat([makeExpr(dr), " ? ", makeExpr(dr), " : ", makeExpr(dr)]); },
function(dr) { return "/*##76##*/" +  cat([makeExpr(dr), " ? ", makeExpr(dr), " : ", makeExpr(dr)]); },
...
function(dr) { return "/*##77##*/" +  cat(["yield ", makeExpr(dr)]); },
function(dr) { return "/*##78##*/" +  cat(["(", "yield ", makeExpr(dr), ")"]); },
...
function(dr) { return "/*##79##*/" +  cat(["[11,12,13,14]",        ".", rndElt(["map", "filter", "some", "sort"]) ]); },
function(dr) { return "/*##80##*/" +  cat(["[15,16,17,18]",        ".", rndElt(["map", "filter", "some", "sort"]), "(", makeFunction(dr), ", ", makeExpr(dr), ")"]); },
function(dr) { return "/*##81##*/" +  cat(["[", makeExpr(dr), "]", ".", rndElt(["map", "filter", "some", "sort"]), "(", makeFunction(dr), ")"]); },

// RegExp replace.  This is interesting for same same reason as array extras.
function(dr) { return "/*##82##*/" +  cat(["'fafafa'", ".", "replace", "(", "/", "a", "/", "g", ", ", makeFunction(dr), ")"]); },

// XML filtering predicate operator!  It isn't lexed specially; there can be a space between the dot and the lparen.
function(dr) { return "/*##83##*/" +  cat([makeId(dr),  ".", "(", makeExpr(dr), ")"]); },
function(dr) { return "/*##84##*/" +  cat([makeE4X(dr),  ".", "(", makeExpr(dr), ")"]); },

// Dot (property access)
function(dr) { return "/*##85##*/" +  cat([makeId(dr),    ".", makeId(dr)]); },
function(dr) { return "/*##86##*/" +  cat([makeExpr(dr),  ".", makeId(dr)]); },

// Index into array
function(dr) { return "/*##87##*/" +  cat([     makeExpr(dr),      "[", makeExpr(dr), "]"]); },
function(dr) { return "/*##88##*/" +  cat(["(", makeExpr(dr), ")", "[", makeExpr(dr), "]"]); },

// Containment in an array or object (or, if this happens to end up on the LHS of an assignment, destructuring)
function(dr) { return "/*##89##*/" +  cat([maybeSharpDecl(), "[", makeExpr(dr), "]"]); },
function(dr) { return "/*##90##*/" +  cat([maybeSharpDecl(), "(", "{", makeId(dr), ": ", makeExpr(dr), "}", ")"]); },

// Sharps on random stuff?
function(dr) { return "/*##91##*/" +  cat([maybeSharpDecl(), makeExpr(dr)]); },

// Functions: called immediately/not
function(dr) { return "/*##92##*/" +  makeFunction(dr); },
function(dr) { return "/*##93##*/" +  cat(["(", makeFunction(dr), ")", "(", makeActualArgList(dr), ")"]); },

// Try to call things that may or may not be functions.
function(dr) { return "/*##94##*/" +  cat([     makeExpr(dr),          "(", makeActualArgList(dr), ")"]); },
function(dr) { return "/*##95##*/" +  cat(["(", makeExpr(dr),     ")", "(", makeActualArgList(dr), ")"]); },
function(dr) { return "/*##96##*/" +  cat([     makeFunction(dr),      "(", makeActualArgList(dr), ")"]); },

// Try to test function.call heavily.
function(dr) { return "/*##97##*/" +  cat(["(", makeFunction(dr), ")", ".", "call", "(", makeExpr(dr), ", ", makeActualArgList(dr), ")"]); },

// Binary "new", with and without clarifying parentheses, with expressions or functions
function(dr) { return "/*##98##*/" +  cat(["new ",      makeExpr(dr),          "(", makeActualArgList(dr), ")"]); },
function(dr) { return "/*##99##*/" +  cat(["new ", "(", makeExpr(dr), ")",     "(", makeActualArgList(dr), ")"]); },

function(dr) { return "/*##100##*/" +  cat(["new ",      makeFunction(dr),      "(", makeActualArgList(dr), ")"]); },
function(dr) { return "/*##101##*/" +  cat(["new ", "(", makeFunction(dr), ")", "(", makeActualArgList(dr), ")"]); },

// Sometimes we do crazy stuff, like putting a statement where an expression should go.  This frequently causes a syntax error.
function(dr) { return "/*##102##*/" +  stripSemicolon(makeLittleStatement(dr)); },
function(dr) { return "/*##103##*/" +  ""; },

// Let expressions -- note the lack of curly braces.
function(dr) { return "/*##104##*/" +  cat(["let ", "(", makeLetHead(dr), ") ", makeExpr(dr)]); },

// Array comprehensions (JavaScript 1.7)
function(dr) { return "/*##105##*/" +  cat(["[", makeExpr(dr), makeComprehension(dr), "]"]); },

// Generator expressions (JavaScript 1.8)
function(dr) { return "/*##106##*/" +  cat([     makeExpr(dr), makeComprehension(dr)     ]); },
function(dr) { return "/*##107##*/" +  cat(["(", makeExpr(dr), makeComprehension(dr), ")"]); },


function(dr) { return "/*##108##*/" +  cat([" /* Comment */", makeExpr(dr)]); },
function(dr) { return "/*##109##*/" +  cat(["\n", makeExpr(dr)]); }, // perhaps trigger semicolon insertion and stuff
function(dr) { return "/*##110##*/" +  cat([makeExpr(dr), "\n"]); },

function(dr) { return "/*##111##*/" +  cat([makeLValue(dr)]); },

// Assignment (can be destructuring)
function(dr) { return "/*##112##*/" +  cat([makeLValue(dr), " = ", makeExpr(dr)]); },
function(dr) { return "/*##112##*/" +  cat([makeLValue(dr), " = ", makeExpr(dr)]); },
function(dr) { return "/*##112##*/" +  cat([makeLValue(dr), " = ", makeExpr(dr)]); },
function(dr) { return "/*##112##*/" +  cat([makeLValue(dr), " = ", makeExpr(dr)]); },

// Destructuring assignment
function(dr) { return "/*##113##*/" +  cat([makeDestructuringLValue(dr), " = ", makeExpr(dr)]); },
function(dr) { return "/*##113##*/" +  cat([makeDestructuringLValue(dr), " = ", makeExpr(dr)]); },
function(dr) { return "/*##113##*/" +  cat([makeDestructuringLValue(dr), " = ", makeExpr(dr)]); },
function(dr) { return "/*##113##*/" +  cat([makeDestructuringLValue(dr), " = ", makeExpr(dr)]); },

// Destructuring assignment with lots of group assignment
function(dr) { return "/*##114##*/" +  cat([makeDestructuringLValue(dr), " = ", makeDestructuringLValue(dr)]); },

// Modifying assignment, with operators that do various coercions
function(dr) { return "/*##115##*/" +  cat([makeLValue(dr), rndElt(["|=", "%=", "+=", "-="]), makeExpr(dr)]); },

// Watchpoints (similar to setters)
function(dr) { return "/*##116##*/" +  cat([makeExpr(dr), ".", "watch", "(", uneval(makeId(dr)), ", ", makeFunction(dr), ")"]); },
function(dr) { return "/*##117##*/" +  cat([makeExpr(dr), ".", "unwatch", "(", uneval(makeId(dr)), ")"]); },

// New-style getter/setter, imperative
function(dr) { return "/*##118##*/" +  cat([makeExpr(dr), ".", "__defineGetter__", "(", uneval(makeId(dr)), ", ", makeFunction(dr), ")"]); },
function(dr) { return "/*##119##*/" +  cat([makeExpr(dr), ".", "__defineSetter__", "(", uneval(makeId(dr)), ", ", makeFunction(dr), ")"]); },
function(dr) { return "/*##120##*/" +  cat(["this", ".", "__defineGetter__", "(", uneval(makeId(dr)), ", ", makeFunction(dr), ")"]); },
function(dr) { return "/*##121##*/" +  cat(["this", ".", "__defineSetter__", "(", uneval(makeId(dr)), ", ", makeFunction(dr), ")"]); },

// Old-style getter/setter, imperative
function(dr) { return "/*##122##*/" +  cat([makeId(dr), ".", makeId(dr), " ", rndElt(["getter", "setter"]), "= ", makeFunction(dr)]); },

// Object literal
function(dr) { return "/*##123##*/" +  cat(["(", "{", makeObjLiteralPart(dr), " }", ")"]); },
function(dr) { return "/*##124##*/" +  cat(["(", "{", makeObjLiteralPart(dr), ", ", makeObjLiteralPart(dr), " }", ")"]); },

// Test js_ReportIsNotFunction heavily.
function(dr) { return "/*##125##*/" +  "(p={}, (p.z = " + makeExpr(dr) + ")())"; },

// Test js_ReportIsNotFunction heavily.
// Test decompilation for ".keyword" a bit.
// Test throwing-into-generator sometimes.
function(dr) { return "/*##126##*/" +  cat([makeExpr(dr), ".", "throw", "(", makeExpr(dr), ")"]); },
function(dr) { return "/*##127##*/" +  cat([makeExpr(dr), ".", "yoyo",   "(", makeExpr(dr), ")"]); },

// Throws, but more importantly, tests js_DecompileValueGenerator in various contexts.
function(dr) { return "/*##128##*/" +  "this.zzz.zzz"; },

// Test eval in various contexts. (but avoid clobbering eval)
// Test the special "obj.eval" and "eval(..., obj)" forms.
function(dr) { return "/*##129##*/" +  makeExpr(dr) + ".eval(" + makeExpr(dr) + ")"; },
function(dr) { return "/*##130##*/" +  "eval(" + uneval(makeExpr(dr)) + ", " + makeExpr(dr) + ")"; },
function(dr) { return "/*##131##*/" +  "eval(" + uneval(makeStatement(dr)) + ", " + makeExpr(dr) + ")"; },
...
function(dr) { return "/*##132##*/" +  "(uneval(" + makeExpr(dr) + "))"; },

// Object.prototype stuff
function(dr) { return "/*##133##*/" +  cat([makeExpr(dr), ".", "hasOwnProperty", "(", uneval(makeId(dr)), ")"]); },
function(dr) { return "/*##134##*/" +  cat([makeExpr(dr), ".", "propertyIsEnumerable", "(", uneval(makeId(dr)), ")"]); },
function(dr) { return "/*##135##*/" +  cat([makeExpr(dr), ".", "isPrototypeOf", "(", makeExpr(dr), ")"]); },
function(dr) { return "/*##136##*/" +  cat([makeExpr(dr), ".", "__lookupGetter__", "(", uneval(makeId(dr)), ")"]); },
function(dr) { return "/*##137##*/" +  cat([makeExpr(dr), ".", "__lookupSetter__", "(", uneval(makeId(dr)), ")"]); },
function(dr) { return "/*##138##*/" +  cat([makeExpr(dr), ".", "valueOf", "(", uneval("number"), ")"]); },

// Constructors.  No "T" -- don't screw with the constructors themselves; just call them.
function(dr) { return "/*##139##*/" +  "new " + rndElt(constructors) + "(" + makeActualArgList(dr) + ")"; },
...
function(dr) { return "/*##140##*/" +  cat([makeFunPrefix(dr), "function", " ", maybeName(dr), "(", makeFormalArgList(dr), ")", makeFunctionBody(dr)]); },
function(dr) { return "/*##140##*/" +  cat([makeFunPrefix(dr), "function", " ", maybeName(dr), "(", makeFormalArgList(dr), ")", makeFunctionBody(dr)]); },
function(dr) { return "/*##140##*/" +  cat([makeFunPrefix(dr), "function", " ", maybeName(dr), "(", makeFormalArgList(dr), ")", makeFunctionBody(dr)]); },
function(dr) { return "/*##140##*/" +  cat([makeFunPrefix(dr), "function", " ", maybeName(dr), "(", makeFormalArgList(dr), ")", makeFunctionBody(dr)]); },
...
function(dr) { return "/*##141##*/" +  "function(q) { return q; }" },

// A generator that does something
function(dr) { return "/*##142##*/" +  "function(y) { yield y; " + makeStatement(dr) + "; yield y; }" },

// A generator expression -- kinda a function??
function(dr) { return "/*##143##*/" +  "(1 for (x in []))"; },
...
function(dr) { return "/*##144##*/" +  "eval" }, // eval is interesting both for its "no indirect calls" feature and for the way it's implemented -- a special bytecode.
function(dr) { return "/*##145##*/" +  "new Function" }, // this won't be interpreted the same way for each caller of makeFunction, but that's ok
function(dr) { return "/*##146##*/" +  "(new Function(" + uneval(makeStatement(dr)) + "))"; },
function(dr) { return "/*##147##*/" +  "Function" }, // without "new"!  it does seem to work...
function(dr) { return "/*##148##*/" +  "gc" },
function(dr) { return "/*##149##*/" +  "Math.sin" },
function(dr) { return "/*##150##*/" +  "Math.pow" },
function(dr) { return "/*##151##*/" +  "/a/gi" }, // in Firefox, at least, regular expressions can be used as functions: e.g. "hahaa".replace(/a+/g, /aa/g) is "hnullhaa"!
function(dr) { return "/*##152##*/" +  "[1,2,3,4].map" },
function(dr) { return "/*##153##*/" +  "[1,2,3,4].slice" },
function(dr) { return "/*##154##*/" +  "'haha'.split" },
function(dr) { return "/*##155##*/" +  "({}).hasOwnProperty" },
function(dr) { return "/*##156##*/" +  "({}).__lookupGetter__" },
function(dr) { return "/*##157##*/" +  cat(["(", makeFunction(dr), ")", ".", "call"]); },
function(dr) { return "/*##158##*/" +  cat(["(", makeFunction(dr), ")", ".", "apply"]); },
function(dr) { return "/*##159##*/" +  cat(["(", makeExpr(dr), ")", ".", "watch"]); },
function(dr) { return "/*##160##*/" +  cat(["(", makeExpr(dr), ")", ".", "__defineSetter__"]); },
];
...
function(dr) { return "/*##161##*/" +  cat([makeId(dr)]); },

// Destructuring
function(dr) { return "/*##162##*/" +  makeDestructuringLValue(dr); },
function(dr) { return "/*##162##*/" +  makeDestructuringLValue(dr); },

// Properties
function(dr) { return "/*##163##*/" +  cat([makeId(dr), ".", makeId(dr)]); },
function(dr) { return "/*##164##*/" +  cat([makeExpr(dr), ".", makeId(dr)]); },
function(dr) { return "/*##165##*/" +  cat([makeExpr(dr), "[", "'", makeId(dr), "'", "]"]); },

// Special properties
function(dr) { return "/*##166##*/" +  cat([makeId(dr), ".", rndElt(specialProperties)]); },

// Certain functions can act as lvalues!  See JS_HAS_LVALUE_RETURN in js engine source.
function(dr) { return "/*##167##*/" +  cat([makeId(dr), "(", makeExpr(dr), ")"]); },
function(dr) { return "/*##168##*/" +  cat(["(", makeExpr(dr), ")", "(", makeExpr(dr), ")"]); },

// Parenthesized lvalues can cause problems ;)
function(dr) { return "/*##169##*/" +  cat(["(", makeLValue(dr), ")"]); },
...
function(dr) { return "/*##170##*/" +  cat(["(", "{ ", makeId(dr), ": ", makeDestructuringLValue(dr), " }", ")"]); },
function(dr) { return "/*##171##*/" +  cat(["(", "{ ", makeId(dr), ": ", makeDestructuringLValue(dr), ", ", makeId(dr), ": ", makeDestructuringLValue(dr), " }", ")"]); },
];
